...
def render_status(paths: Paths, config: ResolvedConfig, env: EnvironmentInfo, as_json: bool) -> str:
    ...
    payload = {
        ...
        "ai_model": config.ai.get("model", "qwen2.5-coder:7b"),
        "ai_ask_model": config.ai.get("ask_model") or config.ai.get("nav_model") or config.ai.get("model", "qwen2.5-coder:7b"),
        "ai_fix_model": config.ai.get("fix_model") or config.ai.get("model", "qwen2.5-coder:7b"),
